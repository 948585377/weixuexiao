...
if($operation == 'GetMiddleData'){
	...
	if ($_W['schooltype']){
		...
		$sql_xszj       = "SELECT count(id) FROM " .GetTableName("students"). " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' AND createdate > '{$starttime}' AND createdate < '{$endtime}' AND (stheendtime >='{$endtime}' or stheendtime = 0) ";
		$sql_xszjGuoqi  = "SELECT count(id) FROM " .GetTableName('students'). " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' AND createdate > '{$starttime}' AND createdate < '{$endtime}' AND stheendtime < '{$endtime}' AND stheendtime != 0 ";
	}else{
