...
if ($operation == 'post') {
	...
	if(keep_Blacklist()){
		$tcourse = pdo_fetch("SELECT kcabbr,kcnumber,start FROM ".GetTableName('tcourse')." WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' AND id = '{$kcid}'");
		$xq = pdo_fetch("SELECT njabbr FROM ".GetTableName('classify')." WHERE sid = '{$tcourse['xq_id']}' ");
		$kccode = $tcourse['kcabbr'].$xq['njabbr'].$tcourse['kcnumber'].date("Ymd",$tcourse['start']).$kcid;
