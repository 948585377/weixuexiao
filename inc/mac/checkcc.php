...
global $_GPC, $_W;
$operation = in_array ( $_GPC ['op'], array ('default','notice', 'device', 'class', 'student','check','teacher','picture','attendence','news','activity','timetable','duty', 'praise','food','log','daylog', 'banner', 'video', 'start',  'users', 'getdate','custom','command','getdevremote','checkap','getroomlist','token','getmessage','postmessage','postread') ) ? $_GPC ['op'] : 'default';
$weid      = $_GPC['i'];
...
if ($operation == 'device') {
	...
	if(!empty($ckmac)) {
		...
		$result['data'] = array(
		                ...
		                'startTime'    => $macset['starttime'],
		                'shutdownTime' => $macset['shutdowntime'],
...
if ($operation == 'class') {
	//获取班级信息
	if(!empty($ckmac)) {
		$class = pdo_fetch("SELECT sid as classId ,sname as className FROM " . tablename($this->table_classify) . " WHERE weid = '{$weid}' And type = 'theclass' And schoolid = {$school['id']} And sid = {$ckmac['bj_id']} ");
		if($class) {
			...
			$classHonours=pdo_fetchall("SELECT honourName,id as honourId,addTime,pictureUrls FROM " . tablename($this->table_classcard_honour) . " WHERE weid = '{$weid}' And schoolid = {$school['id']} And bj_id = {$ckmac['bj_id']} ");
			foreach($classHonours as $key=>$row) {
				foreach(unserialize($row['pictureUrls']) as $k=>$m) {
					$imgarr[$k]=tomedia($m);
				}
				$classHonours[$key]['pictureUrls']=$imgarr;
			}
			$classEpaper=pdo_fetchall("SELECT epaperName,id as epaperId,addTime,pictureUrls FROM " . tablename($this->table_classcard_epaper) . " WHERE weid = '{$weid}' And schoolid = {$school['id']} And bj_id = {$ckmac['bj_id']} ");
			foreach($classEpaper as $key=>$row) {
				foreach(unserialize($row['pictureUrls']) as $k=>$m) {
					$imgarr1[$k]=tomedia($m);
				}
				$classEpaper[$key]['pictureUrls']=$imgarr1;
			}
			$result['data'] = $class;
			$result['data']['classAdvert'] = $classAdvert['classAdvert'];
			$result['data']['classHonours'] = $classHonours;
			$result['data']['classEpaper'] = $classEpaper;
			$result['servertime'] = time();
		} else {
			$result['status'] = -1;
...
if ($operation == 'student') {
	//获取学生信息
	if(!empty($ckmac)) {
		$students = pdo_fetchall("SELECT id as studentId, s_name as studentName FROM " . tablename($this->table_students) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' And bj_id = '{$ckmac['bj_id']}'  ORDER BY id DESC " );
...
if ($operation == 'teacher') {
	...
	if(!empty($ckmac)) {
		...
		foreach($bj_t as $key=>$row) {
			...
			$bzr=pdo_fetch("SELECT sid FROM " . tablename($this->table_classify) . " WHERE sid = '{$ckmac['bj_id']}' and tid='{$row['tid']}' ");
			if(empty($bzr)) {
				$teachers[$key]['classAdviser']=1;
			} else {
				$teachers[$key]['classAdviser']=0;
			}
		}
		$result['status'] = 0;
...
if ($operation == 'news') {
	...
	foreach($list as $key => $row) {
		...
		$data[$key]['id'] = $row['id'];
		$data[$key]['info'] = strip_tags(ihtml_entity_decode($row['content']));
...
if ($operation == 'activity') {
	$time=time();
	$list = pdo_fetchall("SELECT * FROM " . tablename($this->table_groupactivity) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' and FIND_IN_SET('{$ckmac['bj_id']}',bjarray) ORDER BY createtime DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	$total=pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename($this->table_groupactivity) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' and FIND_IN_SET('{$ckmac['bj_id']}',bjarray) ");
	$data=array();
	foreach($list as $key => $row) {
		$data[$key]['id'] = $row['id'];
		$data[$key]['title'] = $row['title'];
		...
		$data[$key]['pictureUrls'] = $imgarr;
		$data[$key]['startTime'] = date('Y-m-d',$row['starttime']);
		$data[$key]['endTime'] = date('Y-m-d',$row['endtime']);
	}
	$result['status'] = "0";
	$result['msg']    = "获取数据成功";
	$result['data']['total']=$total;
...
if ($operation == 'check') {
	...
	if(!empty($ckuser)) {
		...
		if($todaychecklog>1) {
			/*$last=pdo_fetch("SELECT id FROM " . tablename($this->table_classcard_checklog) . " WHERE cardid = '{$_GPC['cardCode']}' And schoolid = '{$schoolid}' ORDER BY id DESC ");
			pdo_update($this->table_classcard_checklog, $data,array('id'=>$last['id']));*/
		} else {
