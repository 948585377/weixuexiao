 ...
 */
		...
		if(keep_Blacklist()){
			$schoolset = pdo_fetch("SELECT is_bingyin,bingyincontent FROM ".GetTableName('schoolset')." WHERE weid = '{$weid}' and schoolid = '{$schoolid}' ");
			//获取病因填写字段内容
        ...
        if(!empty($userid['id'])){
			...
			if($_W['schooltype']){
				if(!empty($_GPC['kcid'])){
					...
					$mykc = pdo_fetch("SELECT id,name FROM " . tablename($this->table_tcourse) . " where schoolid = {$schoolid} And maintid = '{$tid_global}' ORDER BY id DESC LIMIT 0,1 ");
					$kcid = $mykc['id'];
				}
				$kclist = pdo_fetchAll("SELECT id,name FROM " . tablename($this->table_tcourse) . " where schoolid = {$schoolid} And maintid = '{$tid_global}' ORDER BY id DESC");
