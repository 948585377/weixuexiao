 ...
 * @author 高贵血迹
 */global $_W, $_GPC;
   $operation = in_array ( $_GPC ['op'], array ('default','getbjlist','bdxs','skcsign','tkcsign','xskcqdqr','xskcbq','qrjsqd','txsk','signup','newstu','get_kslist','get_all_kslist','delsign_one','getxgtemplte','sqingjia','kcpingjia','newkcpingjia','txkcpj','deletetempstu','get_ks_conent','get_stu_payinfo','get_mysherecard','add_ks_clik','set_rank_list','show_rank_list','make_zl_team','join_zl_team','get_onteam_info','get_onorder_info','again_team','get_kc_shareword','team_info','xu_zd','GetTeamData','CheckKcMingE') ) ? $_GPC ['op'] : 'default';
	...
	if ($operation == 'get_all_kslist')  {
		if (! $_GPC ['schoolid']) {
			...
			foreach ($coursebuy as $key => $value) {
				...
				if($kecheng['OldOrNew'] == 0){
					$condition = " AND date > '{$starttime}' AND date < '{$endtime}'";	
					$kslist = pdo_fetchall("SELECT id,sd_id FROM " . tablename($this->table_kcbiao) . " where schoolid = '{$_GPC['schoolid']}' And kcid = '{$value['kcid']}'  $condition ORDER BY date DESC");
